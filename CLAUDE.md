...
- **Process layout.** `coach.py` runs Flask + the PyWebView overlay in the main process and launches `capture_mono.py` as a **subprocess** (`launch_capture_mono`). `RunState` and `MonoEventAdapter` are constructed **inside the capture_mono subprocess** by `capture_mono._wire_run_state()`, so snapshots delivered to `handle_game_state()` produce `runs`/`decisions` rows directly. The main process reads from the shared SQLite DB (WAL mode). Force-end from the Flask `/api/force-end` route writes outcome directly to the DB (the subprocess RunState's in-memory `_run_closed` flag does not flip — acceptable for now since EndRun snapshots close runs cleanly in the normal flow).
