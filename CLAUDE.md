...
Automated build-refresh pipeline status lives in the bazaar-builds repo. Current tracker-facing contract: bazaar-builds is in `phase: shadow_cron` with `dry_run: true`; scheduled runs default to deterministic `no_llm_shadow`; stats sidecars may be committed in bazaar-builds; tracker catalogs and tracker PRs must not be mutated until `live_cron` is manually enabled. Do not promote to `live_cron` until there are at least 6 healthy bazaardb patch windows, at least 60 calendar days of shadow output, and semantic classifier/provider readiness or an explicit waiver.
