...
| Pipeline phase | `bazaar-builds` is at `phase: shadow_cron` with `dry_run: true`. The Actions cron schedule exists, so scheduled/manual shadow runs can fetch, evaluate, write, upload artifacts, and commit stats sidecars in bazaar-builds. They default to deterministic `no_llm_shadow` and still do not mutate tracker catalogs or open tracker PRs. `live_cron` remains a later gate requiring accumulated healthy shadow history and semantic catalog-review readiness. |
...
Follow-up validation covered Dooley, Karnok, Mak, Pygmalien, and Vanessa in `local_dry_run` with `--mock-llm`, live source fetches, temp-only artifacts, and exit code 0 for each hero. Focused tests passed at that point (`59 passed in 0.39s`); the current `bazaar-builds` tracked test suite is `119 passed` when run as `python -m pytest -q tests`. Each hero produced diff JSON and proposal markdown, no real LLM/API calls occurred, and no checked-in state, catalog, stats sidecar, or tracker catalog files mutated. Artifact review treats mock-mode outputs as operational evidence only. Scheduled `shadow_cron` now defaults to deterministic `no_llm_shadow`; those artifacts are still operational evidence only, with `classification_pending` labels rather than carry/core/support catalog-acceptance semantics.
...
- **Semantic classifier/provider strategy for catalog acceptance**: deterministic/no-LLM is resolved for shadow observation. Before `live_cron` or any catalog-acceptance automation, decide whether to use the existing Claude-backed classifier, an alternate provider such as Gemini or another free/lower-cost provider, provider abstraction before hosted usage, or an explicit operator waiver.
...
- **Pipeline phase is `shadow_cron`**: `bazaar-builds` has been promoted with `dry_run: true`. `implementation` is the off switch, and `local_dry_run` remains the rollback path for artifact-only runs without stats commits. `shadow_cron` persists stats sidecars in bazaar-builds while still avoiding tracker PR/catalog mutation. `live_cron` remains a later gate requiring at least 6 healthy bazaardb patch windows and at least 60 calendar days of shadow output (per subtask 1 §8), plus a semantic classifier/provider decision or explicit waiver.
